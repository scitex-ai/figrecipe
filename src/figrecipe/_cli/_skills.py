...
`_skills/figrecipe/` directory directly.
"""
...
def _skills_root() -> Path:
    """Resolve the bundled `_skills/figrecipe/` directory."""
    import figrecipe

    pkg_dir = Path(figrecipe.__file__).parent
    return pkg_dir / "_skills" / PKG


def _list_skill_files(root: Path) -> list[Path]:
    """All `.md` files under `_skills/figrecipe/` (recursive), excluding SKILL.md."""
    if not root.is_dir():
        return []
...
def skills_group(ctx) -> None:
    """Agent-facing skills bundled with figrecipe.

    \b
    Examples:
      $ figrecipe skills list
      $ figrecipe skills get 01_installation
      $ figrecipe skills install                  # → ~/.scitex/dev/skills/figrecipe/
      $ figrecipe skills install --claude-symlink # also expose to ~/.claude/skills/scitex/
    """
...
def skills_get(name: str, as_json: bool) -> None:
    ...
    Example:
      $ figrecipe skills get 01_installation
      $ figrecipe skills get 02_quick-start --json
    """
    root = _skills_root()
...
@click.option(
    ...
    type=click.Path(),
    default=None,
    help="Destination dir (default: ~/.scitex/dev/skills/figrecipe/).",
)
@click.option(
    "--no-link",
    "no_link",
...
def skills_install(
    ...
    \b
    Default: symlink the entire `_skills/figrecipe/` dir to
    ~/.scitex/dev/skills/figrecipe/ so add/rename/delete in
    source propagates immediately.

    \b
    Example:
      $ figrecipe skills install
      $ figrecipe skills install --claude-symlink
      $ figrecipe skills install --no-link --dest /tmp/figrecipe-skills
    """
