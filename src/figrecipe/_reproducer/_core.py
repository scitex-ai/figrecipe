...
def _replay_call(
    ...
    # Methods that do not replay as a plain axes call (figrecipe helpers,
    # reconstructed objects, seaborn). Match order matters — see _replay_dispatch.
    handler = find_special_handler(method_name)
    if handler is not None:
        return handler(ax, call, result_cache)
    ...
    # Repair arguments that could not survive serialization verbatim. A fixup
    # may also refuse the call outright (legacy tick labels with no authored
    # positions), in which case replaying it would MISLABEL the axis.
    fixed = apply_arg_fixups(ax, method_name, args, kwargs)
    if fixed.skipped:
        return None

    # Call the method
    try:
        return method(*fixed.args, **fixed.kwargs)
...
def _reconstruct_call_args(
    call: CallRecord,
    result_cache: Dict[str, Any],
    coerce_sequences: bool,
) -> tuple:
    """Rebuild a recorded call's ``(args, kwargs)`` from their serialized form."""
