#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# File: figrecipe/_editable/_editable_export.py
...
def export_editable_figure(
    ...
    """
    # Unwrap figrecipe RecordingFigure (.fig / ._fig) or a scitex-style
    # FigureWrapper (.figure); otherwise assume a bare matplotlib Figure.
    mpl_fig = fig
    for _attr in ("figure", "fig", "_fig"):
        _candidate = getattr(fig, _attr, None)
        if _candidate is not None and hasattr(_candidate, "get_axes"):
            mpl_fig = _candidate
            break
